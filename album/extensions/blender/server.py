...
class BlenderConnectThread(threading.Thread):
    args = []
    running = False
    serversocket = None
    ...
    def start(self):
        # Start the thread.
        print('Starting Blender connect thread')
        self.running = True
        self.daemon = True
    ...
    def stop(self):
        # Stop the thread.
        print('Stopping Blender connect thread')
        self.running = False
        self.serversocket.settimeout(0.1)
        self.serversocket.close()
        del self.serversocket
        print('Done stopping Blender connect thread')
...
class BlenderConnection(bpy.types.Operator):
    bl_idname = 'album.connect'
    bl_label = 'Connect to album'
    ...
    def execute(self, context):
        self.thread = BlenderConnectThread()
        self.thread.start()
...
def register():
    bpy.utils.register_class(BlenderConnection)


if __name__ == "__main__":
    register()
    bpy.ops.album.connect()
