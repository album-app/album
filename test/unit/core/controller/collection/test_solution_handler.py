...
class TestSolutionHandler(TestCatalogCollectionCommon):
    ...
    def test_set_cache_paths(self):
        ...
        self.assertEqual(
            Path(config.get_cache_path_download()).joinpath(
                "catalog_name_solution_lives_in", "tsg", "tsn", "tsv"
            ),
            active_solution.installation.data_path
        )
        self.assertEqual(
            Path(config.get_cache_path_app()).joinpath(
                "catalog_name_solution_lives_in", "tsg", "tsn", "tsv"
            ),
        ...
        self.assertEqual(
            Path(config.get_cache_path_tmp_internal()).joinpath(
                "catalog_name_solution_lives_in", "tsg", "tsn", "tsv"
            ),
            active_solution.installation.internal_cache_path
        )
        self.assertEqual(
            Path(config.get_cache_path_tmp_user()).joinpath(
                "catalog_name_solution_lives_in", "tsg", "tsn", "tsv"
            ),
