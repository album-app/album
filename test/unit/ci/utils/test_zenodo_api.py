...
class TestZenodoDeposit(TestZenodoCommon):
    def setUp(self):
        ...
        self.test_offline_zenodo_deposit = ZenodoDeposit(
            self.init_dict, self.base_url, self.access_token
        )

    def test_get_files_id_by_name(self):
        self.assertTrue(
            self.test_offline_zenodo_deposit.get_files_id_by_name("test_file_2") == 2
        )

    def test_list_files_name(self):
        self.assertTrue(
            self.test_offline_zenodo_deposit.list_files_name()
    ...
    def test_create_file(self):
        ...
        self.assertEqual(
            self.test_deposit.files[0].filename, os.path.basename(file.name)
        )
    ...
    def test_delete_file(self):
        ...
        self.test_deposit.create_file(file.name)
        self.assertEqual(
            expected_file_name, self.test_deposit.files[expected_file_name].filename
        )
    ...
    def test_update_file(self):
        ...
        self.assertEqual(len(self.test_deposit.files), 1)
        self.assertEqual(
            self.test_deposit.files[0].filename, os.path.basename(file_new.name)
        )
...
class TestZenodoAPI(TestZenodoCommon):
    def test_validate_response(self):
        ...
        self.assertDictEqual(
            {"test": "test_value"},
            ZenodoAPI.validate_response(response, ResponseStatus.Accepted),
        )
        ...
        self.assertDictEqual(
            {"test": "test_value"},
            ZenodoAPI.validate_response(response, ResponseStatus.Created),
        )

        # case NoContent
        response.status_code = ResponseStatus.NoContent.value
        self.assertTrue(
            bool(ZenodoAPI.validate_response(response, ResponseStatus.NoContent))
        )
        ...
        self.assertTrue(
            ...
            % ResponseStatus.InternalServerError.name
            in str(context.exception)
        )

    def test_deposit_get(self):
        self.assertEqual(
            self.test_deposit.id, self.zenodoAPI.deposit_get(self.test_deposit.id)[0].id
        )
    ...
    def test_deposit_create_with_prereserve_doi(self):
        ...
        self.assertIsNot(
            self.test_deposit2.metadata.prereserve_doi["doi"], "", "doi empty string!"
        )
        self.assertIsNotNone(self.test_deposit2.id, "ID of deposit not set!")
        self.assertIsNotNone(
            self.test_deposit2.metadata.prereserve_doi["doi"], "doi of deposit not set!"
        )
