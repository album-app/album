...
class TestIntegrationClone(TestIntegrationCoreCommon):
    ...
    def test_clone_catalog_template(self):
        ...
        # run
        self.album_controller.clone_manager().clone("template:catalog", target_dir=str(target_dir), name="my_catalog")

        # assert
        self.assertNotIn('ERROR', self.captured_output.getvalue())
        target_path = Path(self.tmp_dir.name).joinpath("my_catalogs", "my_catalog")
        self.assertIn(
            "Initialized new catalog \"my_catalog\" from template \"catalog\" in %s" % target_dir,
