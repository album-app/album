...
class TestIntegrationClone(TestIntegrationCoreCommon):
    ...
    def test_clone_solution_template(self):
        ...
        # run
        self.album_controller.clone_manager().clone(
            "group:name:0.1.0", target_dir=str(target_dir), name="my_solution"
        )
    ...
    def test_clone_catalog_template(self):
        ...
        # run
        self.album_controller.clone_manager().clone(
            "template:catalog", target_dir=str(target_path), name="my_catalog"
        )
    ...
    def test_clone_non_existing_solution(self):
        ...
        with self.assertRaises(LookupError) as e:
            self.album_controller.clone_manager().clone(
                "weirdPath", target_dir=str(Path(self.tmp_dir.name)), name="my_solution"
            )
