...
class TestIntegrationMigrationManager(TestIntegrationCoreCommon):
    ...
    def test_migrate_catalog_collection_single(self, mock_resource_filename):
        ...
        collection_path.return_value = Path(self.tmp_dir.name).joinpath(
            "catalog_collection.db"
        )
        self.album_controller._collection_manager.catalog_collection.get_path = (
            collection_path
        )
        ...
        result = self.check_migration(
            Path(self.tmp_dir.name).joinpath("catalog_collection.db"), check_script
        )
