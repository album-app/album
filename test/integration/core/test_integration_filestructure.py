...
class TestIntegrationFileStructure(TestIntegrationCoreCommon):
    ...
    def check_files_exist(self, base, group, name, version, size, id="0"):
        ...
        self.assertTrue(
            self._link(
                base.joinpath(
                    "catalogs", "cache_catalog", "solutions", group, name, version
                )
            ).exists()
        )
        self.assertEqual(
            base.joinpath("lnk", "pck", id).resolve(),
            get_link_target(
                base.joinpath(
                    "catalogs", "cache_catalog", "solutions", group, name, version
                )
        ...
        self.assertTrue(
            self._link(
                base.joinpath("installations", "cache_catalog", group, name, version)
            ).exists()
        )
        self.assertEqual(
            base.joinpath("lnk", "inst", id).resolve(),
            get_link_target(
                base.joinpath("installations", "cache_catalog", group, name, version)
            ).resolve(),
        )
