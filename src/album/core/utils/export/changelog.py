...
def get_changelog_content(active_solution, catalog, dummy_content: str = None):
    content = '''# Changelog
...
'''
    versions = catalog.get_all_solution_versions(active_solution.coordinates.group,
    ...
    for version in versions:
        ...
        if timestamp:
            ...
            if version.setup.version == active_solution.setup.version and dummy_content:
                ...
                if version.setup['changelog']:
                    ...
                    change = ''
            content += '\n## [%s] - %s\n%s\n' % (version.setup['version'], time, change)
    return content


def create_changelog_file(active_solution: Solution, catalog: Catalog, target_folder: Path):
...
def process_changelog_file(catalog: Catalog, active_solution: Solution, deploy_path: Path):
