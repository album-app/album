...
from album.core.utils.operations.git_operations import create_new_head, add_files_commit_and_push, \
    retrieve_default_mr_push_options, checkout_main, clean_repository, remove_files, remove_tag, add_tag, \
    get_tags, revert
from album.core.utils.operations.resolve_operations import get_attributes_from_string, dict_to_coordinates, \
    as_tag_unversioned, as_tag
...
class DeployManager(IDeployManager):
    ...
    def undeploy(
        ...
        exit_msg = 'Pretending to remove %s from %s...' if dry_run else 'Removing %s from %s...'
        module_logger().info(exit_msg % (solution_to_resolve, catalog_name))

        coordinates = dict_to_coordinates(get_attributes_from_string(solution_to_resolve))
        catalog = self.album.catalogs().get_by_name(catalog_name)
        # check for cache catalog only
        if catalog.is_cache():
            raise RuntimeError("Cannot undeploy from catalog only used for caching! Aborting...")

        with catalog.retrieve_catalog(self.get_download_path(catalog), force_retrieve=True) as repo:
            # load index
            ...
            current_version_tag = ordered_version_tags[0] if len(ordered_version_tags) > 0 else None
            previous_version_tag = ordered_version_tags[1] if len(ordered_version_tags) > 1 else None

            version_to_be_removed = as_tag(coordinates)
            if not version_to_be_removed in ordered_version_tags:
                raise LookupError("Cannot find %s in deployed versions %s" % (version_to_be_removed, ordered_version_tags))
            ...
            if not previous_version_tag:
                ...
                self._remove_db_entry_and_files(repo, coordinates, dry_run, push_options, git_email, git_name)
            else:
                if current_version_tag == version_to_be_removed:
                    # roll back to last version
                    self._remove_db_entry_and_revert_files(repo, coordinates, previous_version_tag, dry_run,
                                                   push_options, git_email, git_name)
                else:
                    # remove tag, update db
                    self._remove_db_entry_and_tag(repo, coordinates, dry_run, push_options, git_email, git_name)

        # refresh the local index of the catalog
        self.album.migration_manager().refresh_index(catalog)

        exit_msg = 'Successfully pretended to remove %s from %s.' if dry_run else 'Successfully removed %s from %s.'
        module_logger().info(exit_msg % (solution_to_resolve, catalog_name))
    ...
    def _deploy_to_direct_catalog(
        ...
        if not dry_run:
            try:
                self._push_directly(active_solution.coordinates(), repo, [solution_root], commit_files, dry_run, push_options, git_email, git_name)
            ...
            module_logger().info("Would commit the changes and push to %s..." % catalog.src())
    ...
    def _deploy_to_request_catalog(
        ...
        self._create_merge_request(active_solution.coordinates(), repo, mr_files, dry_run, push_options, git_email, git_name)
    ...
    @staticmethod
    def _remove_from_downloaded_catalog(catalog: ICatalog, coordinates: ICoordinates, dry_run: bool):
        """Updates the index in the downloaded repository!"""
        if not dry_run:
            catalog.remove(coordinates)
        else:
            module_logger().info("Would remove the solution %s from index..." % coordinates.name())
    ...
    @staticmethod
    def _create_merge_request(coordinates: ICoordinates, repo: Repo, file_paths, dry_run=False, push_option=None,
        ...
        commit_msg = "Adding new/updated %s" % DeployManager.retrieve_head_name(coordinates)
    ...
    @staticmethod
    def _push_directly(coordinates: ICoordinates, repo: Repo, files_to_remove: list, files_to_add: list, dry_run=False, push_option=None,
                       email=None, username=None):
        if push_option is None:
            push_option = []
    ...
    def _remove_db_entry_and_files(self, repo, coordinates, dry_run, push_options, git_email, git_name):

        solution_root = str(self.album.configuration().get_solution_path_suffix_unversioned(coordinates))
        db_index = DefaultValues.catalog_index_file_name.value

        init_msg = 'Would remove files %s, would update %s..' if dry_run else 'Removing files %s, updating %s..'
        module_logger().info(init_msg % (solution_root, db_index))

        if not dry_run:
            self._try_push_remove_tag(coordinates, repo, [solution_root], [db_index], push_options, git_email, git_name)

    def _remove_db_entry_and_revert_files(self, repo, coordinates, previous_version_tag, dry_run, push_options,
                                          git_email, git_name):

        solution_root = str(self.album.configuration().get_solution_path_suffix_unversioned(coordinates))
        db_index = DefaultValues.catalog_index_file_name.value

        init_msg = 'Would revert %s to %s, would update %s..' if dry_run else 'Reverting %s to %s, updating %s..'
        module_logger().info(init_msg % (solution_root, previous_version_tag, db_index))

        if not dry_run:
            revert(repo, previous_version_tag, [solution_root])
            self._try_push_remove_tag(coordinates, repo, [], [db_index], push_options, git_email, git_name)

    def _remove_db_entry_and_tag(self, repo, coordinates, dry_run, push_options, git_email,
                                 git_name):
        db_index = DefaultValues.catalog_index_file_name.value
    ...
    def _try_push_remove_tag(self, coordinates, repo, files_to_remove, files_to_add, push_options, git_email, git_name):
        try:
