...
class RunManager(IRunManager):
    ...
    def run(self, solution_to_resolve: str, run_immediately=False, argv=None):
        ...
        if not resolve_result.catalog:
            ...
            module_logger().debug('solution loaded from catalog: \"%s\"...' % str(coordinates))
        module_logger().debug(
            "Initializing script to run \"%s\"" % coordinates)
        ...
        self.album.event_manager().publish(Event(DefaultValues.before_run_event_name.value), resolve_result.coordinates())

        # runs the queue
        try:
            self.album.script_manager().run_queue(que)
        finally:
            self.album.event_manager().publish(Event(DefaultValues.after_run_event_name.value), resolve_result.coordinates())
    ...
    def load_plugins(self, resolve_result):
        ...
        if 'dependencies' in resolve_result.loaded_solution().setup():
            ...
            if 'plugins' in deps:
                module_logger().debug('Processing solution plugins: %s' % deps['plugins'])
                ...
                for plugin in deps['plugins']:
                    ...
                    for entry_point in available_plugins:
                        if entry_point.name == plugin['id']:
                            module_logger().debug('Found plugin %s, initializing activation..' % plugin['id'])
                            plugin_call = entry_point.load()
                            plugin_args = plugin['args'] if 'args' in plugin else {}
                            from album.api import Album
                            plugin_call(Album(self.album), resolve_result.coordinates(), plugin_args)
                            module_logger().debug('Plugin %s activated.' % plugin['id'])
                            found = True
                            break
                    if not found:
                        raise LookupError('Cannot find plugin %s - please make sure the package providing this plugin '
