...
class CollectionManager(ICollectionManager):
    ...
    def resolve_and_load(self, resolve_solution) -> ICollectionSolution:
        ...
        self.solution_handler.set_cache_paths(resolve_result.loaded_solution(), resolve_result.catalog())
    ...
    def resolve_and_load_coordinates(self, coordinates: ICoordinates) -> ICollectionSolution:
        ...
        catalog = self.album.catalogs().get_by_id(collection_entry.internal()["catalog_id"])
    ...
    def resolve(self, resolve_solution) -> ICollectionSolution:
        ...
        if not Path(resolve_result.path()).exists():
            self.solution_handler.retrieve_solution(resolve_result.catalog(), resolve_result.coordinates())
        ...
        if "doi" in attrs:  # case doi
            ...
            if not solution_entry:
                if "catalog" in attrs:  # resolve in specific catalog
                    catalog_id = self.album.catalogs().get_by_name(attrs["catalog"]).catalog_id()
