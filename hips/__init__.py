...
def download_environment_yaml(active_hips):
    ...
    module_logger.debug('Downloading environment file from %s into %s...' % (environment_file, environment_url))
...
def run_in_environment(active_hips, script):
    ...
    """
    module_logger.debug('run_in_environment: %s...' % active_hips["_environment_path"])
    ...
    module_logger.debug('Running soltuion with subprocess command: %s...' % " ".join(subprocess_args))
