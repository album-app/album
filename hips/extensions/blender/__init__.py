...
def __install_linux():
    ...
    download_path = public_api.download_if_not_exists(active_hips, path_download_linux, download_name_linux)
    app_path = public_api.get_cache_path_app(active_hips)
    public_api.extract_tar(download_path, app_path)
...
def __run_linux():
    active_hips = hips.get_active_hips()
    blender_path = f"{public_api.get_cache_path_app(active_hips)}/{path_run_linux}"
